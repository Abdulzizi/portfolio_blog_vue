  ...
  <AdminLayout>
    ...
    <div v-else>
      ...
      <div class="bg-white border border-black">
        ...
        <form @submit.prevent="handleSubmit" class="p-6">
          <div class="grid grid-cols-1 gap-6">
              ...
              <div class="space-y-3">
                ...
                <div class="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    @click="addTech"
                    :disabled="
                      form.tech_stack.length >= MAX_TECH || !newTech.trim()
                    "
                ...
                <div class="mt-2">
                  <label class="flex items-center">
                    ...
                    <span class="ml-2 text-sm text-gray-700"
                      >I'm still working on this project</span
                    >
                  </label>
                </div>
              </div>
            ...
            <div
              v-if="isEditMode && (project?.created_at || project?.updated_at)"
                ...
                <div v-else class="space-y-2">
                  <div class="relative inline-block">
                    <img
                      ...
                      class="object-cover mx-auto rounded border border-black"
                    />
                    <button
                      type="button"
                      @click="clearThumbnail"
                      class="absolute top-1 right-1 w-6 h-6 bg-red-600 text-white rounded-full text-xs hover:bg-red-700 transition-colors"
                    >
                      ×
                    </button>
                  </div>
      ...
      <div
        ...
        class="mt-8 bg-white border border-black"
      >
        <div class="p-6 border-b border-black">
...
const handleTitleInput = (event) => {
  ...
  // Only auto-generate slug in add mode (not edit mode)
  if (!isEditMode.value) {
    form.value.slug = generateSlug(title);
  }
};

// Watch title changes for real-time slug generation (add mode only)
watch(
  () => form.value.title,
  (newTitle) => {
...
const handleThumbnailUpload = (event) => {
  ...
  if (file) {
    ...
    // Convert to base64 and store directly in form model
    const reader = new FileReader();
    reader.onload = (e) => {
      const base64String = e.target.result;
      form.value.thumbnail = base64String; // Store base64 string instead of file object
...
// Update the handleSubmit function to properly handle files
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      const updateData = {
        title: form.value.title.trim() || project.value.title,
        description: form.value.description.trim() || project.value.description,
        link: form.value.link.trim() || project.value.link,
        slug: form.value.slug.trim() || project.value.slug,
        tech_stack: form.value.tech_stack || project.value.tech_stack,
        start_date: form.value.start_date || project.value.start_date,
        end_date: stillWorking.value ? null : form.value.end_date || null,
        is_published: form.value.is_published
          ? 1
          : 0 || project.value.is_published,
        thumbnail: form.value.thumbnail || project.value.thumbnail,
      };
